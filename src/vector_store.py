...
def _setup_faiss(documents: List[Document], embedding_model, collection_name: str, is_gemini: bool, db_config: dict = None):
    ...
    if documents and not index_loaded:
        logger.info(f"Ingesting {len(documents)} documents into FAISS at {persist_directory}")
...
def _setup_chroma(documents: List[Document], embedding_model, collection_name: str, is_gemini: bool, db_config: dict = None):
    ...
    if documents:
        ...
        logger.info(f"Loading existing Chroma from {persist_directory}")
        vector_store = Chroma(
            collection_name=chroma_collection_name,
...
def add_to_vector_store(vector_store, documents: List[Document], persist_directory: str = None):
    """
    Adds documents to an existing vector store.
    Args:
        persist_directory: Explicit path to save the index to.
    """
    if not vector_store:
        raise ValueError("Vector store is not initialized")
    ...
    if persist_directory:
        vector_store.save_local(persist_directory)
        logger.info(f"Saved update to {persist_directory}")
    else:
        logger.warning("No persist_directory provided, index updated in memory but NOT saved to disk.")

def delete_from_vector_store(vector_store, ids: List[str], persist_directory: str = None):
    """
    ...
    try:
        ...
        if persist_directory:
            ...
            logger.warning("No persist_directory provided, deletion applied in memory but NOT saved to disk.")
    except Exception as e:
