...
class HuggingFaceLLMClient:
    def __init__(self, model_name, model_dir=None, use_4bit=False):
        ...
        # Check device availability: CUDA > MPS (Apple Silicon) > CPU
        has_cuda = torch.cuda.is_available()
        has_mps = torch.backends.mps.is_available() if hasattr(torch.backends, 'mps') else False
        
        # Only use 4-bit quantization on CUDA GPUs
        use_quantization = has_cuda and use_4bit
        
        if use_quantization:
            # CUDA GPU with quantization
            ...
            # Apple Silicon (M1/M2/M3) - use float16 for better memory efficiency
            self.model = AutoModelForCausalLM.from_pretrained(model_name, 
                                                              cache_dir=model_dir,
                                                              torch_dtype=torch.float16,
                                                              )
